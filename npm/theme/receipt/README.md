# @ox-content/theme-receipt

Receipt — A thermal roll — one narrow centred column, dotted leaders and a torn edge — for [Ox Content](https://github.com/ubugeeei-prod/ox-content).
...
`@ox-content/theme-color-*` scheme. About 9.0 kB of CSS, zero JavaScript, zero
runtime dependencies.

```bash
npm install @ox-content/theme-receipt @ox-content/theme-color-tokyo-night
```
...
import receipt from "@ox-content/theme-receipt";
...
export default defineConfig({
  plugins: [
    oxContent({
      srcDir: "docs",
      ssg: { siteName: "My Docs", theme: [receipt, tokyoNight] },
    }),
  ],
});
```
...
```ts
theme: [receipt, tokyoNight, { colors: { primary: "#ff5f56" } }];
```
...
```ts
theme: [receipt, tokyoNight, { tokens: { "motion-base": "200ms", "motion-ease": "linear" } }];
```

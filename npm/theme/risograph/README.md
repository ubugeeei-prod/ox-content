# @ox-content/theme-risograph

Risograph — Misregistered duotone print where the ink channels separate on hover — for [Ox Content](https://github.com/ubugeeei-prod/ox-content).
...
```bash
npm install @ox-content/theme-risograph @ox-content/theme-color-tokyo-night
```
...
import risograph from "@ox-content/theme-risograph";
...
export default defineConfig({
  plugins: [
    oxContent({
      srcDir: "docs",
      ssg: { siteName: "My Docs", theme: [risograph, tokyoNight] },
    }),
  ],
});
```
...
```ts
theme: [risograph, tokyoNight, { colors: { primary: "#ff5f56" } }];
```
...
```ts
theme: [risograph, tokyoNight, { tokens: { "motion-base": "200ms", "motion-ease": "linear" } }];
```

# @ox-content/theme-bauhaus

Bauhaus — Circle, square, triangle and a diagonal that refuses the grid — for [Ox Content](https://github.com/ubugeeei-prod/ox-content).
...
```bash
npm install @ox-content/theme-bauhaus @ox-content/theme-color-tokyo-night
```
...
import bauhaus from "@ox-content/theme-bauhaus";
...
export default defineConfig({
  plugins: [
    oxContent({
      srcDir: "docs",
      ssg: { siteName: "My Docs", theme: [bauhaus, tokyoNight] },
    }),
  ],
});
```
...
```ts
theme: [bauhaus, tokyoNight, { colors: { primary: "#ff5f56" } }];
```
...
```ts
theme: [bauhaus, tokyoNight, { tokens: { "motion-base": "200ms", "motion-ease": "linear" } }];
```

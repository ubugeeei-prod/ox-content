# @ox-content/theme-leather

Leather — Embossed grain and saddle stitching that presses in when touched — for [Ox Content](https://github.com/ubugeeei-prod/ox-content).
...
```bash
npm install @ox-content/theme-leather @ox-content/theme-color-tokyo-night
```
...
import leather from "@ox-content/theme-leather";
...
export default defineConfig({
  plugins: [
    oxContent({
      srcDir: "docs",
      ssg: { siteName: "My Docs", theme: [leather, tokyoNight] },
    }),
  ],
});
```
...
```ts
theme: [leather, tokyoNight, { colors: { primary: "#ff5f56" } }];
```
...
```ts
theme: [leather, tokyoNight, { tokens: { "motion-base": "200ms", "motion-ease": "linear" } }];
```

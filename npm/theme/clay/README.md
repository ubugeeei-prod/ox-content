# @ox-content/theme-clay

Clay — Soft extruded clay that squishes under the pointer — for [Ox Content](https://github.com/ubugeeei-prod/ox-content).
...
```bash
npm install @ox-content/theme-clay @ox-content/theme-color-tokyo-night
```
...
import clay from "@ox-content/theme-clay";
...
export default defineConfig({
  plugins: [
    oxContent({
      srcDir: "docs",
      ssg: { siteName: "My Docs", theme: [clay, tokyoNight] },
    }),
  ],
});
```
...
```ts
theme: [clay, tokyoNight, { colors: { primary: "#ff5f56" } }];
```
...
```ts
theme: [clay, tokyoNight, { tokens: { "motion-base": "200ms", "motion-ease": "linear" } }];
```

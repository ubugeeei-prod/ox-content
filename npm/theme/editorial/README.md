# @ox-content/theme-editorial

Editorial — Magazine typography with drop caps, hairline rules and column reveals — for [Ox Content](https://github.com/ubugeeei-prod/ox-content).
...
`@ox-content/theme-color-*` scheme. About 9.6 kB of CSS, zero JavaScript, zero
runtime dependencies.

```bash
npm install @ox-content/theme-editorial @ox-content/theme-color-tokyo-night
```
...
import editorial from "@ox-content/theme-editorial";
...
export default defineConfig({
  plugins: [
    oxContent({
      srcDir: "docs",
      ssg: { siteName: "My Docs", theme: [editorial, tokyoNight] },
    }),
  ],
});
```
...
```ts
theme: [editorial, tokyoNight, { colors: { primary: "#ff5f56" } }];
```
...
```ts
theme: [editorial, tokyoNight, { tokens: { "motion-base": "200ms", "motion-ease": "linear" } }];
```

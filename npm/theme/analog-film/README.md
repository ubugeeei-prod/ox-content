# @ox-content/theme-analog-film

Analog Film — Grain, halation and sprocket rails with a gentle gate weave — for [Ox Content](https://github.com/ubugeeei-prod/ox-content).
...
`@ox-content/theme-color-*` scheme. About 11.0 kB of CSS, zero JavaScript, zero
runtime dependencies.

```bash
npm install @ox-content/theme-analog-film @ox-content/theme-color-tokyo-night
```
...
import analogFilm from "@ox-content/theme-analog-film";
...
export default defineConfig({
  plugins: [
    oxContent({
      srcDir: "docs",
      ssg: { siteName: "My Docs", theme: [analogFilm, tokyoNight] },
    }),
  ],
});
```
...
```ts
theme: [analogFilm, tokyoNight, { colors: { primary: "#ff5f56" } }];
```
...
```ts
theme: [analogFilm, tokyoNight, { tokens: { "motion-base": "200ms", "motion-ease": "linear" } }];
```

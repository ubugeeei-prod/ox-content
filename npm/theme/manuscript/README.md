# @ox-content/theme-manuscript

Manuscript — A codex page — narrow measure, rubricated heads, and a table of contents that becomes marginalia — for [Ox Content](https://github.com/ubugeeei-prod/ox-content).
...
```bash
npm install @ox-content/theme-manuscript @ox-content/theme-color-tokyo-night
```
...
import manuscript from "@ox-content/theme-manuscript";
...
export default defineConfig({
  plugins: [
    oxContent({
      srcDir: "docs",
      ssg: { siteName: "My Docs", theme: [manuscript, tokyoNight] },
    }),
  ],
});
```
...
```ts
theme: [manuscript, tokyoNight, { colors: { primary: "#ff5f56" } }];
```
...
```ts
theme: [manuscript, tokyoNight, { tokens: { "motion-base": "200ms", "motion-ease": "linear" } }];
```

# @ox-content/theme-blur-glass

Blur Glass — Frosted backdrop-blur layers that resolve out of a soft haze — for [Ox Content](https://github.com/ubugeeei-prod/ox-content).
...
`@ox-content/theme-color-*` scheme. About 8.9 kB of CSS, zero JavaScript, zero
runtime dependencies.

```bash
npm install @ox-content/theme-blur-glass @ox-content/theme-color-tokyo-night
```
...
import blurGlass from "@ox-content/theme-blur-glass";
...
export default defineConfig({
  plugins: [
    oxContent({
      srcDir: "docs",
      ssg: { siteName: "My Docs", theme: [blurGlass, tokyoNight] },
    }),
  ],
});
```
...
```ts
theme: [blurGlass, tokyoNight, { colors: { primary: "#ff5f56" } }];
```
...
```ts
theme: [blurGlass, tokyoNight, { tokens: { "motion-base": "200ms", "motion-ease": "linear" } }];
```

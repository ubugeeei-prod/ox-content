# @ox-content/theme-zine

Zine — Photocopied and taped together — nothing square to the page — for [Ox Content](https://github.com/ubugeeei-prod/ox-content).
...
`@ox-content/theme-color-*` scheme. About 9.1 kB of CSS, zero JavaScript, zero
runtime dependencies.

```bash
npm install @ox-content/theme-zine @ox-content/theme-color-tokyo-night
```
...
import zine from "@ox-content/theme-zine";
...
export default defineConfig({
  plugins: [
    oxContent({
      srcDir: "docs",
      ssg: { siteName: "My Docs", theme: [zine, tokyoNight] },
    }),
  ],
});
```
...
```ts
theme: [zine, tokyoNight, { colors: { primary: "#ff5f56" } }];
```
...
```ts
theme: [zine, tokyoNight, { tokens: { "motion-base": "200ms", "motion-ease": "linear" } }];
```

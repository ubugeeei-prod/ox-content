# @ox-content/theme-noir

Noir — One hard key light and a steep falloff — shapes picked out of the dark — for [Ox Content](https://github.com/ubugeeei-prod/ox-content).
...
```bash
npm install @ox-content/theme-noir @ox-content/theme-color-tokyo-night
```
...
import noir from "@ox-content/theme-noir";
...
export default defineConfig({
  plugins: [
    oxContent({
      srcDir: "docs",
      ssg: { siteName: "My Docs", theme: [noir, tokyoNight] },
    }),
  ],
});
```
...
```ts
theme: [noir, tokyoNight, { colors: { primary: "#ff5f56" } }];
```
...
```ts
theme: [noir, tokyoNight, { tokens: { "motion-base": "200ms", "motion-ease": "linear" } }];
```

# @ox-content/theme-pixel

Pixel — Chunky 8-bit surfaces, hard offset shadows and stepped motion — for [Ox Content](https://github.com/ubugeeei-prod/ox-content).
...
```bash
npm install @ox-content/theme-pixel @ox-content/theme-color-tokyo-night
```
...
export default defineConfig({
  plugins: [
    oxContent({
      srcDir: "docs",
      ssg: { siteName: "My Docs", theme: [pixel, tokyoNight] },
    }),
  ],
});
```
...
```ts
theme: [pixel, tokyoNight, { tokens: { "motion-base": "200ms", "motion-ease": "linear" } }];
```

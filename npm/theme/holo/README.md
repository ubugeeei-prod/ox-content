# @ox-content/theme-holo

Holo — Iridescent foil that shifts hue as panels tilt and slide — for [Ox Content](https://github.com/ubugeeei-prod/ox-content).
...
```bash
npm install @ox-content/theme-holo @ox-content/theme-color-tokyo-night
```
...
export default defineConfig({
  plugins: [
    oxContent({
      srcDir: "docs",
      ssg: { siteName: "My Docs", theme: [holo, tokyoNight] },
    }),
  ],
});
```
...
```ts
theme: [holo, tokyoNight, { colors: { primary: "#ff5f56" } }];
```
...
```ts
theme: [holo, tokyoNight, { tokens: { "motion-base": "200ms", "motion-ease": "linear" } }];
```

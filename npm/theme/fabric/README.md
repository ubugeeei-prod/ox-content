# @ox-content/theme-fabric

Fabric — Woven texture, stitched seams and soft cloth-fold reveals — for [Ox Content](https://github.com/ubugeeei-prod/ox-content).
...
`@ox-content/theme-color-*` scheme. About 10.0 kB of CSS and generated JavaScript, zero
runtime dependencies.

```bash
npm install @ox-content/theme-fabric @ox-content/theme-color-tokyo-night
```
...
import fabric from "@ox-content/theme-fabric";
...
export default defineConfig({
  plugins: [
    oxContent({
      srcDir: "docs",
      ssg: { siteName: "My Docs", theme: [fabric, tokyoNight] },
    }),
  ],
});
```
...
```ts
theme: [fabric, tokyoNight, { colors: { primary: "#ff5f56" } }];
```
...
```ts
theme: [fabric, tokyoNight, { tokens: { "motion-base": "200ms", "motion-ease": "linear" } }];
```

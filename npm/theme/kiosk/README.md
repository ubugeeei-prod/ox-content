# @ox-content/theme-kiosk

Kiosk — Platform signage — a departure-board sidebar, banded heads and arrows — for [Ox Content](https://github.com/ubugeeei-prod/ox-content).
...
```bash
npm install @ox-content/theme-kiosk @ox-content/theme-color-tokyo-night
```
...
import kiosk from "@ox-content/theme-kiosk";
...
export default defineConfig({
  plugins: [
    oxContent({
      srcDir: "docs",
      ssg: { siteName: "My Docs", theme: [kiosk, tokyoNight] },
    }),
  ],
});
```
...
```ts
theme: [kiosk, tokyoNight, { colors: { primary: "#ff5f56" } }];
```

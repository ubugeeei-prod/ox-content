...
Swiss — International Typographic Style — a hard grid, rules, and precise slides — for [Ox Content](https://github.com/ubugeeei-prod/ox-content).
...
```bash
npm install @ox-content/theme-swiss @ox-content/theme-color-tokyo-night
```
...
```ts
theme: [swiss, tokyoNight, { colors: { primary: "#ff5f56" } }];
```
...
```ts
theme: [swiss, tokyoNight, { tokens: { "motion-base": "200ms", "motion-ease": "linear" } }];
```

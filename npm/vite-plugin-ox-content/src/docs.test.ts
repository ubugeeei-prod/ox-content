...
describe("writeDocs", () => {
  it("removes stale generated files from the previous manifest", async () => {
    ...
    await expect(fs.readFile(path.join(outDir, "beta.md"), "utf-8")).resolves.toMatchSnapshot();
  });
  ...
  it("does not delete files that were never tracked by the manifest", async () => {
    ...
    await expect(fs.readFile(path.join(outDir, "manual.md"), "utf-8")).resolves.toMatchSnapshot();
  });
  ...
  it("uses the configured base path for generated nav metadata", async () => {
    ...
    await expect(fs.readFile(path.join(outDir, "nav.ts"), "utf-8")).resolves.toMatchSnapshot();
  });
  ...
  it("writes typedoc nested files and nav tree when pathStrategy is typedoc", async () => {
    ...
    await expect(
      fs.access(path.join(outDir, "default/variables/version.md")),
...
describe("generateMarkdown", () => {
  ...
  it("passes stats and generated-by options to generated Markdown", () => {
    const docs: ExtractedDocs[] = [
      {
        file: "/repo/src/utils.ts",
        entries: [
    ...
    const markdown = generateMarkdown(
      docs,
      resolveDocsOptions({ renderStats: false, renderGeneratedBy: false })!,
    );

    expect(markdown["index.md"]).toMatchSnapshot();
  });

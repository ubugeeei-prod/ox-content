...
describe("Svelte HTML host browser client", () => {
  ...
  it("bundles a minimal browser entry without Vite, Node or native dependencies", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "ox-svelte-html-client-"));
    try {
      await fs.writeFile(
        path.join(root, "entry.js"),
        [
          'import { createSvelteHtmlHostDomRenderer, initSvelteHtmlHost } from "@ox-content/vite-plugin-svelte/html-host/client";',
          "const controller = initSvelteHtmlHost({",
          "  initIslands: () => ({ destroy() {} }),",
          "  modules: {},",
          "  mount: { mode: 'render' },",
          "});",
          "console.log(typeof controller, typeof createSvelteHtmlHostDomRenderer);",
        ].join("\n"),
      );
      ...
      const output = await build({
        ...
        resolve: {
          alias: {
            "@ox-content/vite-plugin-svelte/html-host/client": fileURLToPath(
  ...
  it("creates explicit DOM renderers for fresh render and hydration", () => {
    expect(typeof createSvelteHtmlHostDomRenderer({ mode: "render" })).toBe("function");
    expect(typeof createSvelteHtmlHostDomRenderer({ mode: "hydrate" })).toBe("function");
  });

  it("loads distinct document-scoped modules for reused local component names", async () => {
    const calls: string[] = [];
    const hydrate = createSvelteHtmlHostLazyHydrate({
      modules: {
        "./docs/a/Chart.svelte": () => ({ default: "chart-a" }),
        "./docs/b/Chart.svelte": () => ({ default: "chart-b" }),
      },
    ...
    hydrate(
      element({ oxIsland: "Chart", oxModule: "./docs/a/Chart.svelte", oxExport: "default" }),
      {},
    );
    hydrate(
      element({ oxIsland: "Chart", oxModule: "./docs/b/Chart.svelte", oxExport: "default" }),
      {},
    );
    await settle();

    expect(calls).toEqual(["./docs/a/Chart.svelte:chart-a", "./docs/b/Chart.svelte:chart-b"]);
  });
  ...
  it("reports unknown modules, load failures, missing exports and render failures", async () => {
    const errors: SvelteHtmlHostClientError[] = [];
    const hydrate = createSvelteHtmlHostLazyHydrate({
      modules: {
        "./Bad.svelte": () => Promise.reject(new Error("network gone")),
        "./Missing.svelte": () => ({}),
        "./Render.svelte": () => ({ default: "component" }),
      },
      render: () => {
    ...
    hydrate(element({ oxIsland: "Unknown", oxModule: "./Unknown.svelte" }), {});
    hydrate(element({ oxIsland: "Bad", oxModule: "./Bad.svelte" }), {});
    hydrate(element({ oxIsland: "Missing", oxModule: "./Missing.svelte" }), {});
    hydrate(element({ oxIsland: "Render", oxModule: "./Render.svelte" }), {});
    await settle();
  ...
  it("cancels stale pending loads and disposes mounted islands exactly once", async () => {
    ...
    let disposals = 0;
    const hydrate = createSvelteHtmlHostLazyHydrate({
      modules: { "./Slow.svelte": () => pending.promise },
      render: () => {
        renders += 1;
        return () => {
          disposals += 1;
    ...
    const staleDispose = hydrate(element({ oxIsland: "Slow", oxModule: "./Slow.svelte" }), {});
    staleDispose();
    pending.resolve({ default: "slow" });
    await settle();
    expect(renders).toBe(0);

    const dispose = hydrate(element({ oxIsland: "Slow", oxModule: "./Slow.svelte" }), {});
    await settle();
    dispose();
    dispose();
  ...
  it("passes authored slots without treating self-closing SSR output as children", async () => {
    ...
    const hydrate = createSvelteHtmlHostLazyHydrate({
      modules: {
        "./Slot.svelte": () => ({ default: "slot" }),
        "./Self.svelte": () => ({ default: "self" }),
      },
    ...
    const slot = element(
      {
        oxIsland: "Slot",
        oxModule: "./Slot.svelte",
        oxSsr: "true",
        oxContent: "<em>authored</em>",
      },
      "<strong>SSR output</strong>",
    );
    const selfClosing = element(
      { oxIsland: "Self", oxModule: "./Self.svelte", oxSsr: "true" },
      "<strong>SSR output</strong>",
    );

...
use crate::{
    JsA11y, JsGitContributor, JsReaderChrome, JsSsgBarePage, JsSsgConfig, JsSsgExternalizedAssets,
...
fn contributors_cache() -> &'static Mutex<HashMap<(String, String), Vec<JsGitContributor>>> {
    static CACHE: OnceLock<Mutex<HashMap<(String, String), Vec<JsGitContributor>>>> =
        OnceLock::new();
...
fn git_head(root: &std::path::Path) -> Option<String> {
    let output =
        Command::new("git").arg("-C").arg(root).args(["rev-parse", "HEAD"]).output().ok()?;
    if !output.status.success() {
        return None;
    }
...
pub fn get_git_contributors(file_path: String, root: Option<String>) -> Vec<JsGitContributor> {
    ...
    if let Some(head) = head.as_ref() {
        let cache = contributors_cache().lock().unwrap_or_else(|error| error.into_inner());
    ...
    if let Some(head) = head {
        let mut cache = contributors_cache().lock().unwrap_or_else(|error| error.into_inner());

...
// The circular reveal is opt-in and ships as its own section, so the toggle
// falls back to an immediate switch when it is not on the page.
const toggleTheme = (event) => {
  const next = getTheme() === "dark" ? "light" : "dark";
  if (typeof applyThemeTransition === "function") {
    void applyThemeTransition({ event, nextTheme: next, apply: () => setTheme(next) });
  } else {
    setTheme(next);
  }
};

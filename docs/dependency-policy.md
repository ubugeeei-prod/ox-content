...
- npm advisories run through `tools/scripts/check-npm-advisories.mjs`.
- The npm gate fails on new `high` or `critical` advisories unless an item is explicitly listed in `tools/config/dependency-policy.json`.
...
- npm license policy lives in `tools/config/dependency-policy.json` and is checked by `tools/scripts/check-npm-licenses.mjs`.

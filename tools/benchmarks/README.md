...
- **JS comparison sweep** in `bundle-size/` — runs Ox Content alongside
  ...
  large input sizes. Trigger via `node tools/benchmarks/bundle-size/parse-benchmark.mjs` (see
...
- **CommonMark conformance sweep** in `commonmark-conformance/` — scores every
  ...
  `node tools/benchmarks/commonmark-conformance/run.mjs`.
- **Polonius borrow-checker comparison** in `polonius-borrowck/` — measures what
  ...
  Trigger via
  `node tools/benchmarks/polonius-borrowck/run.mjs --toolchain nightly-2026-08-09`.
...
`tools/scripts/render-benchmark-tables.mjs` reads to fill the `CommonMark` column in
...
- **`ferromark`** (parse and render) — builds its arena AST for the parse row
  ...
  [is a source fork of Ox Content v3.2.3](https://github.com/sebastian-software/ferromark/blob/v2.1.2/UPSTREAM.md),
...
`tools/benchmarks/corpus/<project>/`. Today the script tracks:
...
are enforced by `node tools/benchmarks/bundle-size/check-budgets.mjs` on the head

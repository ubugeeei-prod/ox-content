 ...
 *
 * Usage:
 *   node tools/benchmarks/scale/run.mjs [--suite build|og|features|all]
...
function parseOptions(argv) {
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    if (arg === "--suite") {
      ...
      console.log(
        "Usage: node tools/benchmarks/scale/run.mjs [--suite build|og|features|all]\n" +
          "                                    [--pages N] [--iterations N] [--json PATH]",
      );
      process.exit(0);
    } else {
...
async function main() {
  ...
  if (!existsSync(join(HERE, "node_modules"))) {
    throw new Error(
      "tools/benchmarks/scale has no node_modules; run `vp install` from the repository root first",
    );
  }

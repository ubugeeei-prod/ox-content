...
const baseFileByFile = collectBaseFileMap(baseRef);
...
for (const file of checkedFiles) {
  ...
  const baseLines = countBaseLines(baseRef, baseFileByFile.get(file) ?? file);
...
function collectBaseFileMap(base: string): Map<string, string> {
  const files = new Map<string, string>();

  if (base !== "HEAD") {
    const mergeBaseDiff = runGit(
      ["diff", "--name-status", "--find-renames", "--diff-filter=ACMR", `${base}...HEAD`],
      { allowFailure: true },
    );
    if (mergeBaseDiff.status === 0) {
      addNameStatusMappings(files, splitLines(mergeBaseDiff.stdout));
    } else {
      const directDiff = runGit(
        ["diff", "--name-status", "--find-renames", "--diff-filter=ACMR", `${base}..HEAD`],
        { allowFailure: true },
      );
      if (directDiff.status === 0) {
        addNameStatusMappings(files, splitLines(directDiff.stdout));
      }
    }
  }

  if (!process.env.CI) {
    addNameStatusMappings(
      files,
      runGitLines(["diff", "--name-status", "--find-renames", "--diff-filter=ACMR", "HEAD"]),
    );
  }
...
function addNameStatusMappings(files: Map<string, string>, lines: string[]): void {
  for (const line of lines) {
    ...
    if ((status.startsWith("R") || status.startsWith("C")) && secondPath) {
      files.set(secondPath, firstPath);
      continue;
    }
    files.set(firstPath, firstPath);
  }
}
